...
public class Board {
    ...
    private  char calculateNextState(int row, int col){return  anyRulefor(row,col) ? AliveCell : DeadCell;}

    private boolean anyRulefor(int row, int col) {return  rules.stream().anyMatch(rule -> rule.check(row,col));}

    private boolean is(int expected, int... values) { return stream(values).anyMatch(v->v==expected);}
